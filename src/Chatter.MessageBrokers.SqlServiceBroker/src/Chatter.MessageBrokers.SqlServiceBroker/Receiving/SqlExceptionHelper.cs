namespace Chatter.MessageBrokers.SqlServiceBroker.Receiving
{
    ...
    {
        /// <summary>
        /// Determines if a SqlException is transient by leveraging the error number
        /// </summary>
        /// <param name="errorNumber">The error number from the <see cref="SqlError"/></param>
        /// <returns></returns>
        /// <remarks>Source code from here: https://github.com/dotnet/efcore/blob/main/src/EFCore.SqlServer/Storage/Internal/SqlServerTransientExceptionDetector.cs</remarks>
        public static bool IsErrorNumberTransient(int errorNumber)
        {
            switch (errorNumber)
            {
