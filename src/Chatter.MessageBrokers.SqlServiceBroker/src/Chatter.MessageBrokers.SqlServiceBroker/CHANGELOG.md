...
- On the `netstandard2.0` target, `System.Data.SqlClient` does not expose `BeginTransactionAsync`, `CommitAsync`, or `RollbackAsync`; the `netstandard2.0` build uses synchronous transaction fallbacks. `net8.0` and `net10.0` retain the async transaction paths.
