...
On receive, the library filters by message type: only `//Chatter/BrokeredMessage` and `DEFAULT` are surfaced to handlers; `http://schemas.microsoft.com/SQL/ServiceBroker/EndDialog` messages are acknowledged and ended, and every other type is discarded. A discarded message — a Service Broker `Error`, a message of a type this package does not accept, or an accepted type with a null body — has its conversation ended as part of settling it: on the `RECEIVE`'s own transaction under a transactional mode, so the two commit together; under `TransactionMode.None` there is no receive transaction, so the `RECEIVE` has already autocommitted and the `END CONVERSATION` follows it as a separate autocommit, and a process that dies between the two can leave that one endpoint open. `http://schemas.microsoft.com/SQL/ServiceBroker/Error` messages are additionally logged at `Error`, with the Service Broker error code and description projected as their own structured log fields. The code is parsed as a 32-bit integer from the error body and logged as that integer re-rendered — never as peer text. Only printable letters, marks, numbers, punctuation, symbols and spaces are logged from the description, anything else becomes a space, and it is capped at 3000 characters. A missing body logs the `<no error payload>` placeholder for both fields; a body that is not valid UTF-16, whose code does not parse as a 32-bit integer, or that cannot otherwise be read logs the `<unreadable error payload>` placeholder for both fields instead. See [ADR-0037](https://github.com/brenpike/Chatter/blob/master/docs/adr/0037-a-terminal-receive-outcome-ends-its-conversation-and-a-deterministic-sql-fault-is-not-retried.md). The receiver also handles dialog lifecycle (ack = `END CONVERSATION`, nack = transaction rollback, deadletter = resend to the configured dead-letter service path).
