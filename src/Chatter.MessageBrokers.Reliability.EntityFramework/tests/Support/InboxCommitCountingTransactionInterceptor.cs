...
namespace Chatter.MessageBrokers.Reliability.EntityFramework.Tests.Support
{
    ...
    /// inbox is given.
    ///
    /// Both the synchronous and the asynchronous committing hook increment, because which one runs is decided by
    /// the caller that commits, not by the inbox.
    ///
    /// The interception point is the commit itself, not the save, so a SaveChangesAsync that flushes into an open
    /// transaction leaves <see cref="CommitCount"/> untouched. A save counter could not tell a flush from a commit,
    /// which is the whole distinction a test using this hook is drawing.
    ///
    /// A test asserting <see cref="CommitCount"/> is zero is vacuous on its own - an interceptor that was never
    /// wired up also counts zero - so such a test MUST go on to commit and observe the count rise.
    /// </summary>
