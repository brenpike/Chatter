...
namespace Chatter.MessageBrokers.Reliability.EntityFramework.Tests.Support
{
    ...
    /// same MessageId primary key and the same concurrency token the package ships.
    ///
    /// The claim path needs a provider that supplies a real ambient transaction. The EF Core InMemory provider
    /// supplies none, so a claim attempted over it meets BrokeredMessageInbox's no-transaction refusal; every fact
    /// that reaches a claim runs here instead.
    /// </summary>
    ...
    public sealed class InboxClaimSqliteHarness : IDisposable, IAsyncDisposable
    {
        ...
        /// or replacing a provider service, for instance - before they are built.
        /// </summary>
