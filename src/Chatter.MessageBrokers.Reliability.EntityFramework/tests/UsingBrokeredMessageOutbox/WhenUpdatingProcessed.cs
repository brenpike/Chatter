...
namespace Chatter.MessageBrokers.Reliability.EntityFramework.Tests.UsingBrokeredMessageOutbox
{
    public class WhenUpdatingProcessed : Testing.Core.Context
    {
        ...
        // INVARIANT: the claim states its own 'still unprocessed' predicate rather than inheriting one from the change
        // tracker. OutboxMessageConfiguration maps ProcessedFromOutboxAtUtc as a concurrency token, so EF emits the
        // entry's ORIGINAL value as the predicate on the claiming update. A tracked message keeps the null it was
        // loaded with, but Update on a DETACHED one sets original from current, which would make the predicate read
        // '= the stamp just written' and match no row. The CHANGE TRACKER entry is the discriminating observation and
        // the store is not: the claim is staged and nothing saves it here, so the stored row reads unprocessed either
        // way. The message is detached deliberately so the fact reports UpdateProcessedDate's own behaviour rather
        // than the timing of whatever loaded the message.
        [Fact]
        ...
        // INVARIANT: the write bypasses the change tracker and the ProcessedFromOutboxAtUtc concurrency token. The
        // message staged here still carries the processed stamp as its CURRENT value against a null ORIGINAL, so a
        // tracked SaveChanges would re-emit the very 'still unprocessed' predicate that a lost race just failed - and
        // would commit the claim if it now matched. Recording the attempt must neither throw nor disturb that tracked
        // state, and must leave the
