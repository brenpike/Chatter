...
- `WithReliabilityRetention<TContext>(Action<EntityFrameworkReliabilityOptions>)` — one door covering both tables, so it does not matter which reliability behaviour was registered first. A non-positive `InboxDeduplicationWindow`, `ProcessedOutboxRetention` or `PurgeInterval` is refused with `ArgumentOutOfRangeException` at registration rather than at the first purge. The last call wins; the retention purge itself is an internal hosted service registered once, and this option is the only door to it. A purge pass that throws is logged and the loop waits for the next one, so a `TContext` whose model maps neither entity does not take the host down. An inbox marker with no `ReceivedByInboxAtUtc` is never purged, and an outbox row is eligible only once it carries `ProcessedFromOutboxAtUtc`, so an unprocessed row cannot be deleted while it still has work to do.
...
- The unit of work's failure path now runs both terminal steps — rollback and dispose — through one guarded helper, under `CancellationToken.None` so that a cancelled operation still rolls back. A cleanup failure is logged at Warning and the exception that caused the failure is the one rethrown. The guarantee is exception fidelity only: the causal exception survives. It does not resurrect the transaction — a provider whose own disposal aborts still leaves it attached. `BeginAsync` stays outside the guard, so a failure to begin propagates untouched, and the success path's disposal stays unguarded, where a failure is itself the news.
