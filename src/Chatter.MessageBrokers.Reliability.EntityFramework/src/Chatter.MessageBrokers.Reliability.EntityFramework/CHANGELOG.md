...
- `WithReliabilityRetention<TContext>(Action<EntityFrameworkReliabilityOptions>)` — one door covering both tables, so it does not matter which reliability behaviour was registered first. A non-positive `InboxDeduplicationWindow`, `ProcessedOutboxRetention` or `PurgeInterval` is refused with `ArgumentOutOfRangeException` at registration rather than at the first purge. The last call wins; the retention purge itself is an internal hosted service registered once, and this option is the only door to it. A pass deletes in chunks — at most a thousand rows per `DELETE`, oldest first, repeating until a chunk comes back short — so a first pass over a table that has been accumulating for months issues many bounded statements instead of one unbounded one that would escalate to a table lock on the inbox or outbox the durability path writes through. The chunk size is fixed and not configurable; the contract is unchanged, in that every row past its retention is still deleted by the pass that finds it. A purge pass that throws is logged and the loop waits for the next one, so a `TContext` whose model maps neither entity does not take the host down. An inbox marker with no `ReceivedByInboxAtUtc` is never purged, and an outbox row is eligible only once it carries `ProcessedFromOutboxAtUtc`, so an unprocessed row cannot be deleted while it still has work to do.
