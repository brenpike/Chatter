...
**The inbox refuses to claim outside a unit of work.** `ReceiveViaInbox` throws an `InvalidOperationException` naming `TContext` instead of flushing, unless a unit of work began the transaction it would flush into — so both when the context holds no active transaction, and when the active transaction is one you opened yourself. Without a transaction the flush would commit on its own, and a failure anywhere between that commit and the handler's work would leave a marker suppressing a message nothing ever handled. Inside a transaction you opened, the claim would land somewhere neither undo behind it reaches: the rollback that clears the change tracker and the commit that withholds itself over an unsettled claim both act only on a transaction a unit of work began, so your own rollback would leave the claim in that context's identity map for the next lookup to find and skip a message nothing handled. **Registering through `WithInboxBehavior<TContext>()` keeps you on the granted path** — that call registers the matching unit of work, and the order in [Reliability Behavior Order](#reliability-behavior-order) puts `UnitOfWorkBehavior` outside `InboxBehavior`, so the inbox always claims inside a transaction the unit of work began. Nesting is fine too: a unit of work that adopts a transaction another unit of work began still claims, because ownership is recorded against the transaction itself rather than the scope. Otherwise, register the inbox through `WithInboxBehavior<TContext>()`, or run the call inside a unit of work's `ExecuteAsync` rather than inside a transaction of your own. Why the claim is confined this way is recorded in [ADR-0035](https://github.com/brenpike/Chatter/blob/master/docs/adr/0035-a-rolled-back-unit-of-work-reconciles-its-contexts-change-tracker.md).
...
That ownership bounds the inbox as well as the commit: **the inbox refuses to claim inside a transaction the unit of work adopted**. A transaction a unit of work begins is recorded as owned, and `ReceiveViaInbox` refuses to claim into any other — a claim in a transaction you complete yourself would be undone by neither the rollback above nor the unsettled-claim refusal. Dispatching through the inbox inside your own transaction therefore throws rather than deduplicating silently; see [The cost of claiming first](#the-cost-of-claiming-first). A unit of work nested inside another *unit of work's* transaction is unaffected, since that transaction is already owned.
