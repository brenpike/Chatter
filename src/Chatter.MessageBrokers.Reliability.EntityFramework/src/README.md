...
**Both deploy directions are safe, in either order.** An old binary reading rows a new one has been claiming into sees an ordinary deferred row, exactly as a failed attempt would have left it; a new binary reading rows an old one wrote sees `NULL` or a past instant, and both are claimable.
