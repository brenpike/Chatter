...
- **Token acquisition** — for explicit credentials the factory constructs the corresponding `Azure.Identity` credential type directly (`ClientSecretCredential`, `ClientCertificateCredential`, `InteractiveBrowserCredential`), parsing the tenant ID and authority host out of the supplied `authority` URL. With a certificate, it is resolved from the X509 store (`CurrentUser`/`LocalMachine`) by thumbprint (throwing if not found). When no explicit credential is provided, it returns a `DefaultAzureCredential`.
