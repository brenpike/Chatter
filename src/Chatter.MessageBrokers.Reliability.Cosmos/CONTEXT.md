...
**Confirmation Failure**: One drain attempt whose brokered message WAS published and whose delivered stamp then failed. It is a proper sub-kind of Drain Failure — every Confirmation Failure is a Drain Failure, not every Drain Failure is one — and the only kind that costs a republish: the batch is never checkpointed, so the same document re-surfaces and publishes again on every pass, at real broker traffic and real request units. Like any Drain Failure it is NOT a Drain Outcome. It is known from the Outbox Relay's own control flow, at a site reachable only AFTER a successful publish, never inferred from what an exception was. _Avoid_: stamp failure, delivery failure.

**Drain Suspension**: The Outbox Relay's response to consecutive Confirmation Failures on ONE Lease Token of ONE Change-Feed Source Identity — its state is scoped to a single relay processor, so a Lease Token another source reports under the same name is a different lease and is never conflated with it: it stops handing that lease's documents to the drain and takes exactly the same fail-closed exit a publish failure takes, so the batch is not checkpointed and nothing is lost. It requires no successful write — which is the whole reason it is the available answer when the store is refusing the very stamps that are failing — and it halts nothing: no change-feed processor and no hosted service is stopped. It is left by letting exactly ONE probe drain through once the Suspension Window has elapsed, and is lifted outright, with the lease's state evicted, by a confirmation that succeeds. Its threshold and window are a safety bound rather than a tuning parameter, so neither is configurable. It is not the Message Brokers context's Circuit Breaker, which is a receive-side Recovery policy. _Avoid_: circuit breaker, halt, pause, backoff.
...
- The Drain Outcome vocabulary stays CLOSED at *admitted*, *skipped* and *dropped*; neither mechanism adds a fourth value. A Drain Failure, a Confirmation Failure and an Undeliverable Outbox Document each record no Drain Outcome at all — a new concept in this context is a new term, never a new outcome value.
