...
**Outbox Document Contract**: The per-document sibling of the Monitored-Container Contract — the document facts the Outbox Relay verifies for each admitted document, from that document's own persisted bytes ALONE and before any I/O: that it carries a destination, that it carries a persisted body, that its message context materializes into a context at all, that a content type is resolvable from the document or that context, and that the messaging system that context names is persisted as a string — the shape the publish reads it as, so a non-string one names an infrastructure that can never be read, while an absent, null or blank one is no violation at all and stays a host concern. Every violation is named in ONE failure, for the same reason the container contract names all of its own; and a violation marks that one document undeliverable rather than failing the drain, so the lease advances past a document whose own bytes prove redraining it would fail identically. It is evaluated only on the verbatim reconstruction path — an Outbox Body Resolver OWNS the message it publishes, so a document handed to one is never verified against this contract. _Avoid_: poison policy, give-up policy, publishability check.

**Undeliverable Outbox Document**: An Outbox Document the Outbox Relay PROVED it can never publish, advanced out of `pending` to `undeliverable` — a THIRD terminal status alongside `delivered`, and like `delivered` not `pending`, so the give-up stamp's own change-feed event never redrains it. Unlike a delivered document it carries NO time-to-live and is never removed: it is the evidence of the defect that produced it and stays inspectable indefinitely. It records NO Drain Outcome — it never resolved to a publish decision, the same reason a failed publish records none — so it is neither a *dropped* document (which was admitted, resolved to no brokered message, and was marked delivered) nor a fourth outcome value. Nor is it the Message Brokers context's deadletter Settlement, which answers an inbound delivery to broker infrastructure. _Avoid_: dead-letter, quarantine, discard, poison document.
...
**Drain Failure**: One Outbox Relay drain attempt that faulted, reported against the Change-Feed Source Identity and the Lease Token it faulted under. It is not a Drain Outcome: an Outcome is how a document *resolved*, a Failure is an attempt that never resolved, so a Failure is never a fourth Drain Outcome value and that vocabulary stays closed. _Avoid_: drain error, failed outcome.

**Confirmation Failure**: One drain attempt whose brokered message WAS published and whose delivered stamp then failed. It is a proper sub-kind of Drain Failure — every Confirmation Failure is a Drain Failure, not every Drain Failure is one — and the only kind that costs a republish: the batch is never checkpointed, so the same document re-surfaces and publishes again on every pass, at real broker traffic and real request units. Like any Drain Failure it is NOT a Drain Outcome. It is known from the Outbox Relay's own control flow, at a site reachable only AFTER a successful publish, never inferred from what an exception was. Its counterpart is the Confirmation Receipt — a stamp that failed is a Confirmation Failure, a stamp that landed mints a receipt, and a drain that attempted no stamp at all is neither. _Avoid_: stamp failure, delivery failure.

**Confirmation Receipt**: The evidence that a status write LANDED on the monitored container — the value ONE stamp of an Outbox Document's `/status` path mints, and, disjoined across a batch's documents, that batch's evidence that the confirmation path is up. A delivered stamp and an Undeliverable Outbox Document's give-up stamp mint one ALIKE: both are the same write on the same path, and what a Drain Suspension measures is whether that WRITE PATH is up, not whether a publish happened. It is what lifts a Drain Suspension, and the only thing that does — a drain that reached its end having performed no confirming write carries no receipt and lifts nothing. It records NO Drain Outcome, for the same reason a Confirmation Failure records none: it is a fact about the write, not about how a document resolved, so the closed *admitted* / *skipped* / *dropped* vocabulary gains no fourth value. _Avoid_: confirmation flag, success flag — and *drain result*, which is already reserved on Drain Outcome, a receipt being an outcome of nothing.

**Drain Suspension**: The Outbox Relay's response to consecutive Confirmation Failures on ONE Lease Token of ONE Change-Feed Source Identity — its state is scoped to a single relay processor, so a Lease Token another source reports under the same name is a different lease and is never conflated with it: it stops handing that lease's documents to the drain and takes exactly the same fail-closed exit a publish failure takes, so the batch is not checkpointed and nothing is lost. RAISING it requires no successful write — which is the whole reason it is the available answer when the store is refusing the very stamps that are failing — and it halts nothing: no change-feed processor and no hosted service is stopped. It is left by letting exactly ONE probe drain through once the Suspension Window has elapsed, and is lifted outright, with the lease's state evicted, ONLY on presentation of a Confirmation Receipt — never on a drain having merely reached its end, since absence of a Confirmation Failure is not presence of a confirmation. *Consecutive* is therefore measured over confirmation OUTCOMES and not over batch ARRIVALS: a batch that performed no confirming write is not an event the count sees at all, so it neither lifts the suspension nor resets the count. Its threshold and window are a safety bound rather than a tuning parameter, so neither is configurable. It is not the Message Brokers context's Circuit Breaker, which is a receive-side Recovery policy. _Avoid_: circuit breaker, halt, pause, backoff.
...
**Lease Token**: The change-feed lease a batch was delivered for, and the Outbox Relay's partition-progress dimension: batch size and batch count are recorded once per batch against it, so an idle lease with nothing pending stays distinguishable from a stalled one that is not advancing. It is a partition-key-range id of its OWN monitored container, so it is unique only WITHIN one Change-Feed Source Identity — two sources co-resident on one host routinely report the same token. Every measurement therefore carries BOTH, and so does every Drain Suspension. _Avoid_: partition id, processor name (the processor name keys a whole Change-Feed Source Identity, not one lease — which is exactly why the two are named together rather than one standing in for the other).
...
- The Outbox Document Contract and Drain Suspension share NO state: the contract is a total, pure evaluation over one document's own bytes and keeps no counter of any kind, while a Drain Suspension counts consecutive Confirmation Failures against one Change-Feed Source Identity's Lease Token and never reads a document.
- A Drain Suspension is scoped to ONE Change-Feed Source Identity: each relay processor holds its own, so a suspension raised on one source's Lease Token neither refuses another source's batches nor has its count evicted by another source's successful confirmation, even when both sources report a Lease Token of the same name.
- A Drain Suspension takes PRECEDENCE over an Outbox Document Contract evaluation: the suspension check precedes the document loop, so a suspended lease evaluates no contracts, stamps nothing, and marks no document undeliverable.
- A change-feed batch that performed NO confirming write is neither a lift path nor a reset path for a Drain Suspension: an empty batch, and one whose every document the pending-outbox gate skipped, mint no Confirmation Receipt, so the lease's consecutive Confirmation Failure count is left exactly as it was and any open suspension stays open. The receipt is accumulated per BATCH and presented ONCE, after the batch's documents, rather than per document — so a document that confirmed cleanly ahead of a wedging one in the same batch never lifts on its own: the batch takes its fail-closed exit before any receipt is presented.
- The Drain Outcome vocabulary stays CLOSED at *admitted*, *skipped* and *dropped*; neither mechanism adds a fourth value. A Drain Failure, a Confirmation Failure, a Confirmation Receipt and an Undeliverable Outbox Document each record no Drain Outcome at all — a new concept in this context is a new term, never a new outcome value.
