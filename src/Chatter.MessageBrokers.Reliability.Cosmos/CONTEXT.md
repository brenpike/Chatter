...
**Drain Phase**: Whether a Drain Failure struck before or after the drained Outbox Document's brokered message left for the broker. It is derived from the Outbox Relay's own progress — its publish returned, or it did not — never from the failure's type, because the relay is the thing that publishes, so it always knows whether it did. The phase decides which bounded give-up governs the failure: a pre-publish one the opt-in Poison Policy, a post-publish one the always-on Unconfirmed Publish. A run of consecutive failures that mixes both phases is governed by the post-publish cap from its first publish onward, so it can only ever end as an Unconfirmed Publish. _Avoid_: drain stage, failure kind, classification.

**Poison Policy**: The Standalone Outbox Relay's opt-in escape from an Outbox Document that fails deterministically BEFORE its brokered message is published. It governs the pre-publish Drain Phase only: a failure raised after the publish elects the always-on Unconfirmed Publish instead, and is never stamped poisoned, because the message did go out. After a configured number of consecutive pre-publish Drain Failures of one document IDENTITY — the (id, recovered partition key) pair the poison stamp itself targets, so two Outbox Documents sharing a message id in different logical partitions never share a count — that document is advanced to a non-pending poisoned status the relay's admission gate no longer admits and its batch checkpoints, so the Lease Token advances past it and the documents behind it drain. The policy does not classify the Drain Failure it counts: every failure other than a cancellation advances the count, a downstream broker, auth, or throttling outage included, so an enabled policy under a sustained outage gives up on a lease's backlog serially rather than waiting the outage out. The count is process-local — it is held only for one relay instance's lifetime, so it resets on restart and fragments across lease rebalancing and across hosts sharing one Change-Feed Source Identity. It is off by default, a successful drain clears that identity's count, and a document given up on is never deleted and carries no time-to-live — it stays in the container, inspectable, as the evidence of what stalled the relay, until an operator re-drives it by patching its status back to pending. _Avoid_: dead-letter, quarantine, discard.

**Unconfirmed Publish**: The terminal state of an Outbox Document whose brokered message was published but whose delivered stamp never landed within the configured number of consecutive post-publish Drain Failures of that document IDENTITY. It is a document STATUS, not a fourth Drain Outcome value — the publish happened, so the Outcome was already *admitted* and that vocabulary stays closed. It is distinct from a poisoned document, which was never published, and from a delivered one, whose confirmation landed and which is therefore scheduled for self-purge: an unconfirmed one is never deleted and carries no time-to-live, so it stays inspectable as the evidence of a delivery nobody could confirm. Unlike the Poison Policy it is always on and has no off switch, because the cost it bounds — one more publish on every change-feed pass — is paid whether or not anyone opted in. Re-driving it by patching its status back to pending publishes a DUPLICATE, since the message already went out, so what to investigate is whatever is failing the delivered stamp. Both Outbox Relay variants reach it. _Avoid_: dead-letter, quarantine, discard.
