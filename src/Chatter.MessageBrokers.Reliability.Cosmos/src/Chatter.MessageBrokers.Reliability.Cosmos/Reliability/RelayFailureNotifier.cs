...
namespace Chatter.MessageBrokers.Reliability.Cosmos
{
    ...
    internal sealed class RelayFailureNotifier
    {
        ...
        internal Task OnChangeFeedErrorAsync(string leaseToken, Exception exception)
        {
            ...
            {
                // INVARIANT: a failure of the notification is swallowed rather than propagated, and there is no second
                // reporting attempt: the sink that just faulted is the only one this notifier has, so re-reporting
                // through it could only fault again. Observability may never break delivery.
            }
