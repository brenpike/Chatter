...
- an `OutboxProcessingIntervalInMilliseconds` below `-1` is rejected by `Task.Delay` and faults `BrokeredMessageOutboxProcessor`, because that await sits outside the `try`/`catch` guarding a poll pass. The two values `Task.Delay` *accepts* fail silently rather than loudly: `0` polls the outbox as fast as the store will answer, and `-1` (`Timeout.Infinite`) leaves an enabled processor waiting forever after its first pass.
- a `ConcurrentHalfOpenAttempts` below `1` reaches the `new SemaphoreSlim(...)` in the `CircuitBreaker` constructor the first time the breaker is resolved, and `SemaphoreSlim` itself throws — its `maxCount` must be greater than `0`.
