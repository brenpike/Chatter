...
Delivery is **at-least-once, not exactly-once**: a publish that throws leaves the row unprocessed so a later poll retries it once its backoff has elapsed, and a publish that succeeds followed by a mark/commit failure — or a second host instance polling the same durable store concurrently — can dispatch the same message a second time. A handler that is not naturally idempotent should sit behind the Inbox on the receiving side to absorb that duplicate.
