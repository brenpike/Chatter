...
No value in this section is checked while the options are being built. A value this module cannot run with surfaces from the component that actually reads it: an `OutboxProcessingIntervalInMilliseconds` that `Task.Delay` rejects faults `BrokeredMessageOutboxProcessor`, because that await sits outside the `try`/`catch` guarding a poll pass; a `ConcurrentHalfOpenAttempts` below `1` reaches the `new SemaphoreSlim(...)` in the `CircuitBreaker` constructor the first time the breaker is resolved; a `TransactionMode` naming no member of the enum surfaces as a raw configuration-binder exception; and `ReceiverOptions.MaxConcurrentCalls` below `1` raises an `InvalidOperationException` naming the receiver when that receiver initializes. Issue #423 tracks build-time validation.
