...
### Configured values their sinks cannot run with are refused at build time
...
**A value of the right type that its runtime sink cannot run with is refused during `Build()`, by this module.** The refusal is a `ConfiguredValueRefusedException` (`Chatter.MessageBrokers.Exceptions`) — this module's own named failure for a semantic case, in this module's vocabulary rather than the binder's or the sink's. It carries `OptionName` (the offending property, qualified by its options type), `RefusedValue`, `RequiredBound` — stated in terms of the sink the bound was derived from — and `ConfigurationPath`, the section the refusing builder actually resolved, including a custom section name the builder was retargeted onto (`MustNameTheRefusedPropertyValueBoundAndResolvedSectionPathWhenAConfiguredValueIsRefused`, `MustNameTheResolvedCustomSectionPathWhenAConfiguredValueIsRefusedThroughACustomSectionName`). `ConfigurationPath` is null when the refusing builder resolved no section of its own, and the option name still identifies the property in that case.

Validation is its own phase, between resolving the options graph and publishing it, and it walks the FINALIZED graph — after the outermost bind, before any registration. Two consequences follow. A value configured for a NESTED option is reached even though the nested builders have no configuration section of their own, because such a value only lands on the graph after the parent bind. And a refusal leaves the `IServiceCollection` untouched: nothing from the graph is registered, not even the options the publish step would have written first (`MustRefuseANestedCircuitBreakerValueAndPublishNothingWhenTheParentSectionCarriesIt`, `MustRefuseANestedReliabilityValueAndPublishNothingWhenTheParentSectionCarriesIt`).

These are the values that are refused, and the sink each bound was derived from:
...
**Values that are deliberately still accepted.** A value is refused because a named sink cannot run with it, never because it looks odd, so these stay legal:
