...
**The outbox drain reparents; it does not break the chain.** With diagnostics on, `OutboxProcessor` publishes each drained row under its own send span parented to the `traceparent` persisted with that row, then writes **that span's** context onto the outgoing message. A downstream receive therefore parents to the drain span rather than directly to the write span, and the trace reads write → drain → receive: one extra hop, and it is the hop that actually put the message on the broker, minutes after the write and in another process. A row that carries no persisted context — one written while diagnostics were off, or received over a path that propagates none — starts a fresh root instead, with the drain loop's ambient activity attached as a link rather than promoted to parent, because the poll did not cause the message.

The reparenting happens **only when diagnostics are opted into**. With them off, on the metrics-only path, and when the drain span is sampled out, nothing is written and the persisted `traceparent` rides out unchanged.
