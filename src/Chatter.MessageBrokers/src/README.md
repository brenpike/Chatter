...
`InboxBehavior<>` acts on a command dispatched with an `IMessageBrokerContext`, which a Brokered Message Receiver supplies; a command dispatched from outside a received handler passes through. It gates only the **Delivery Entry** — the first message it sees on that delivery's context in the current receive attempt. Each receive attempt has its own Delivery Entry, so a retry is gated again even when the handler builds a fresh Command. A Command your handler dispatches in-process with `context.InMemory()` belongs to the same delivery, so it passes through the Inbox and runs. A delivered Command, and every Command it dispatches in-process, is therefore fully covered. A broker-received event is not itself deduplicated, since the Inbox is a Command behavior; the first Command its handler(s) dispatch in-process in each receive attempt is the one the Inbox gates, and a later Command in that same attempt is at-least-once on retry or redelivery rather than gated. A direct call to the receiver's public `DispatchReceivedMessageAsync` is outside this guarantee, since it does not start a new receive attempt. A behavior you register outside `InboxBehavior<>` that dispatches a command before it calls the next behavior sends that command through the Inbox first; that command then becomes the Delivery Entry, and the delivered command runs again on a retry or redelivery. The behaviors this package ships dispatch nothing before the next behavior. See [ADR-0041](https://github.com/brenpike/Chatter/blob/master/docs/adr/0041-the-inbox-gates-the-message-the-delivery-admitted-not-every-command-that-carries-its-context.md); deduplicating the whole delivery rather than only its Delivery Entry is tracked in [#539](https://github.com/brenpike/Chatter/issues/539). A received message without a `MessageId` makes the in-memory Inbox throw `ArgumentException`, and the handler does not run.
