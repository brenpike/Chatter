...
A denied drain **returns**: it publishes nothing, stamps nothing and spends no dispatch attempt (`MustDispatchNothingWhenTheDrainClaimIsDenied`, `MustNotSpendADispatchAttemptWhenTheDrainClaimIsDenied`). A drain that dies mid-publish needs no cleanup, and there is **no lease to expire and no reaper to run**: the claim only pushes the row's due time one backoff ahead, so the ordinary due gate takes the row again once that instant passes, exactly as it would after a failed attempt.
