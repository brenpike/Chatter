...
### No option value is checked at build time

No value in this section is checked while the options are being built. A value this module cannot run with surfaces from the component that actually reads it: an `OutboxProcessingIntervalInMilliseconds` below `-1` is rejected by `Task.Delay` and faults `BrokeredMessageOutboxProcessor`, because that await sits outside the `try`/`catch` guarding a poll pass — and the two values `Task.Delay` *accepts* fail silently rather than loudly, `0` polling the outbox as fast as the store will answer and `-1` (`Timeout.Infinite`) leaving an enabled processor waiting forever after its first pass; a `ConcurrentHalfOpenAttempts` below `1` reaches the `new SemaphoreSlim(...)` in the `CircuitBreaker` constructor the first time the breaker is resolved; a `TransactionMode` naming no member of the enum surfaces as a raw configuration-binder exception; and `ReceiverOptions.MaxConcurrentCalls` below `1` raises an `InvalidOperationException` naming the receiver when that receiver initializes. Issue #423 tracks build-time validation.
