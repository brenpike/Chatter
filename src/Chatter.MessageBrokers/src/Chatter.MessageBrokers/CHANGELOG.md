...
- `ReliabilityOptionsValidationException` (namespace `Chatter.MessageBrokers.Reliability.Configuration`) — thrown while the reliability options are being built when `OutboxProcessingIntervalInMilliseconds` is below zero, naming every offending value in one aggregated message. The floor is what `BrokeredMessageOutboxProcessor` can actually poll with: it awaits `Task.Delay(interval, token)` OUTSIDE the `try`/`catch` that guards a poll pass, so a value `Task.Delay` rejects faults the whole background service rather than costing one pass. `0` is allowed — polling that aggressively is the operator's call. `-1` is rejected deliberately even though `Task.Delay` accepts it as `Timeout.Infinite`: an enabled processor that waits forever after its first pass is a disable by inference, and disabling the processor is expressible only through `EnableOutboxPollingProcessor`. Validation is source-agnostic — a fluent `WithOutboxPollingProcessor(-1)` is refused exactly as a configured one is — and runs on the finalized instance from both finalize points, `ReliabilityOptionsBuilder.Build()` and `MessageBrokerOptionsBuilder.Build()`, so the parent `Chatter:MessageBrokers` section cannot slip a value past the sub-builder. It also runs when `EnableOutboxPollingProcessor` is `false`: what is validated is the instance that was built, not whether anything currently reads it, because guarding on the processor being enabled is how an invalid value survives to the deployment that later turns it on (#296).
