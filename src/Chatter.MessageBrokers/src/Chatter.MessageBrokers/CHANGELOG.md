...
- **Nested options are no longer registered with the `IServiceCollection` until the whole options graph is finalized.** `MessageBrokerOptionsBuilder.AddReliabilityOptions(...)`, `AddRecoveryOptions(...)` and `RecoveryOptionsBuilder.WithCircuitBreaker(...)` used to `Build()` the sub-builder as the fluent call ran and register its result there and then, so the container carried a `ReliabilityOptions`, `RecoveryOptions` or `CircuitBreakerOptions` registration before the parent builder had bound its own configuration section over the graph. Each of the four options builders in this package — `MessageBrokerOptionsBuilder`, `ReliabilityOptionsBuilder`, `RecoveryOptionsBuilder` and `CircuitBreakerOptionsBuilder` — now splits into an internal `Resolve()`, which produces the finalized options without touching the container, and an internal `Publish(TOptions)`, which is that builder's single registration site; `Build()` is `Resolve()` followed by `Publish()`, and a parent resolves and binds first, then publishes each nested options instance off its own finalized graph, so every entry point reaches one instance per options type. Pinned by `MustNotRegisterAnyServiceWhenResolved` on each of the four builders, and by `MustNotRegisterNestedOptionsBeforeBuildWhenFluentNestedOptionsUsed` and `MustNotRegisterCircuitBreakerOptionsBeforeBuildWhenWithCircuitBreakerUsed`. `Resolve()` and `Publish()` are internal on all four builders; no existing public signature, type or accessibility changed — the publishing entry points (`ReliabilityOptionsBuilder.Build()`, `RecoveryOptionsBuilder.Build()`, `CircuitBreakerOptionsBuilder.Build()` and every `FromConfig` overload) stay public, and `MessageBrokerOptionsBuilder.Build()` stays internal as it already was (#423).
- **A fluent call on any of this package's four options builders now records a value and registers nothing — every registration a builder performs happens in its publish step.** `RecoveryOptionsBuilder.UseNoDelayRecovery()`, `UseExponentialDelayRecovery(int)`, `UseConstantDelayRecovery(int)` and `UseRouteToErrorQueueRecoveryAction()` were the last fluent methods still writing to the container as they were called, each performing its `IRetryDelayStrategy` or `IMaxReceivesExceededAction` `Replace` on the spot. They now record which delay strategy was asked for (with its argument) and whether the route-to-error-queue action was asked for, and `Publish` performs the identical registrations — pinned by `MustResolveNoDelayRetryWhenUseNoDelayRecoveryComposedThroughTheParentBuild`, `MustResolveExponentialDelayRetryWhenUseExponentialDelayRecoveryComposedThroughTheParentBuild`, `MustResolveConstantDelayRetryWhenUseConstantDelayRecoveryComposedThroughTheParentBuild` and `MustResolveErrorQueueDispatcherWhenUseRouteToErrorQueueRecoveryActionComposedThroughTheParentBuild`, which each configure through the parent builder and resolve the strategy or action back out of the built provider. The guarantee is guarded across the WHOLE public fluent surface rather than method by method: `MustLeaveTheContainerEmptyForEveryFluentMethodOnMessageBrokerOptionsBuilder`, `MustLeaveTheContainerEmptyForEveryFluentMethodOnReliabilityOptionsBuilder`, `MustLeaveTheContainerEmptyForEveryFluentMethodOnRecoveryOptionsBuilder` and `MustLeaveTheContainerEmptyForEveryFluentMethodOnCircuitBreakerOptionsBuilder` enumerate every public instance method on their builder by reflection, exclude only the publish path (`Build`, `Resolve`, `Publish`, `FromConfig`) and members declared by `object`, invoke each one over a fresh `ServiceCollection` and assert it stays empty. That exclusion list is a positive allowlist, so a fluent method added later is guarded from the day it is written, and a parameter shape the sweep cannot synthesise fails the test naming that method instead of silently skipping it. This is a statement about the four builders named above and nothing else (#423).
