...
- Built Options is shared with the Azure Service Bus context, which applies the same invariant to its own Service Bus Options through an `InternalsVisibleTo` grant on the registration helper defined here. The invariant governs which INSTANCE every resolution path returns, not which SOURCE wins, so it leaves the Options Binding Precedence divergence between the two contexts untouched.
