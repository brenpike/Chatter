...
**Reliability-Pair Lifetime Contract**: when a consumer supplies a single custom concrete satisfying both facets of a reliability pair (`IBrokeredMessageOutbox` + `IPollableOutboxStore`; `IBrokeredMessageInbox` + `IInboxDeduplicator`), it must register that concrete under the primary facet at `Scoped` or `Singleton` lifetime. The framework derives the secondary-facet registration at the same lifetime so both always resolve to the same instance (split-store is unrepresentable). A `Transient` custom primary is rejected at registration time because DI cannot guarantee same-instance resolution across two Transient resolutions. A consumer that registers both facets independently as separate descriptors pointing to different concretes owns ensuring the two registrations agree; the framework treats this as deliberate consumer intent and does not merge them.
