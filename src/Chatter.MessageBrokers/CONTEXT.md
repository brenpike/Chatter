...
**Outcome**: What a caller reports back to the Circuit Breaker's state store against the Admission it was issued — a success or a failure — never a command. The store adjudicates it: a success can only ever CLOSE the circuit and a failure can only ever OPEN it, so which transition an Outcome warrants is the store's to derive from the Admission's Verdict, not the caller's to choose. The store answers whether THAT report is what moved the circuit, which is the only signal the caller needs.
_Avoid_: "transition command", and naming a specific one (open/close/increment) as something a caller invokes — the store exposes none, which is what makes reporting an Outcome the only way the circuit moves.

**Episode**: The generation of one circuit state. Every transition begins a new episode, and progress belongs to the episode it was admitted under: a half-open trial's success is recorded against its own episode, and a success whose episode has ended is discarded rather than counted toward closing a later one. An Outcome reported against an ended episode changes nothing at all — it neither closes nor re-opens the circuit, and its exception does not become the store's last exception.
