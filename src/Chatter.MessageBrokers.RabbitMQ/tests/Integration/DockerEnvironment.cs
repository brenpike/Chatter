...
{
    // Discovery-time Docker reachability probe. The integration fixture needs Docker to start the RabbitMQ
    // container; when Docker is absent OR the daemon is not actually responding, the integration facts are
    // SKIPPED (never failed) so a plain `dotnet test` on a Docker-free machine stays green. The probe issues a
    // real Docker `/_ping` over the resolved endpoint — a connectable socket file is NOT sufficient (e.g. a WSL
    // /var/run/docker.sock accepts connections even when no daemon serves it), so only a successful daemon ping
    // reports available. The result is cached for the test assembly. Mirrors the SQL Service Broker /
    // Azure Service Bus DockerEnvironment probe exactly.
    internal static class DockerEnvironment
    {
        ...
        public const string SkipReason =
            "Docker is not available (daemon not reachable); the RabbitMQ container cannot be started. " +
