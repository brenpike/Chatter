        ...
        private async Task DispatchToHandlersWithDiagnostics<TMessage>(TMessage message, IMessageHandlerContext messageHandlerContext) where TMessage : IMessage
        {
            ...
            using (var activity = ChatterDiagnostics.StartDispatch<TMessage>(ChatterTelemetryTags.DispatchKinds.Event))
            {
                ...
                catch (Exception e)
                {
                    ...
                    // WhenChatterTracingIsOptedInto.MustNotMarkTheSpanAsFailedWhenTheCallerCancelledTheEventDispatch
                    // and MustRecordOneDispatchDurationWithoutAnErrorTypeWhenTheCallerCancelledTheEventDispatch, which
                    // go red when that clause is deleted. Rationale: ADR-0040.
