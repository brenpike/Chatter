...
- A query's span name and `chatter.message.type` are the query's own type: the `TQuery` argument for `Query<TQuery, TResult>`, and the runtime type of the query for `Query<TResult>(IQuery<TResult>)` — never `IQuery<TResult>`. A query type that implements `IQuery<>` for two result types reports both under one name. Queries follow the same caller-requested cancellation rule as commands and events (ADR-0040). A `Query<TResult>` dispatch that fails because no handler invoker can be built for its runtime type, such as a value-type query, is logged once, rethrown unchanged, and emits a span and a measurement marked with `error.type`; only a null query emits no telemetry. Each distinct query type produces its own `chatter.cqrs.dispatch.duration` series under `chatter.message.type`, so an application that creates query types dynamically should use its telemetry SDK's cardinality limits. No consumer code change is needed. ADR-0010 (#529).
...
- The exception message from `ThrowOnDuplicateCommandHandlers()` now describes the scan order Scrutor 7.0.0 actually uses — assembly load order and the enumeration order of each assembly's loadable types, which the scan collects into a set — instead of the assembly-defined type order it named before. The reword itself changed no behaviour: the rules for which handler is registered, and for when the check throws, are the same as before. The scan order those rules read is not. Where two handlers compete for one command, the replace strategy still keeps the last one scanned, and under 7.0.0 that can be a different handler than under 3.3.0 — which one wins was unspecified in both versions, never a guarantee (ADR-0017). Call `ThrowOnDuplicateCommandHandlers()` to find out at composition time whether any command has competing handlers (#394).

## [0.18.0] - 2026-09-24

### Changed

- Histogram bucket-boundary advice (`InstrumentAdvice`) is now published on every build; previously only the `net10.0` build published it. The README's `net8.0` workaround (register the view yourself) is gone (#395).
- The `exception` span event is now always written by `Activity.AddException`, so a registered `ActivityListener.ExceptionRecorder` can always supply or override its tags (#395).
