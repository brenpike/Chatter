...
`GetOrAdd`, `GetOrDefault` and `GetOrNew` all gate on `TryGet<T>`, so a value of another type stored under the `typeof(T).FullName` key is reported absent and is **overwritten** by the value each of them then creates and stores. A mismatch cannot arise from the type-keyed overloads alone — `Include<T>(T)`, `Get<T>()` and `TryGet<T>(out T)` all derive the key from `T` — but `Include<T>(string, T)` will write any value under any string, including one equal to some `typeof(T).FullName`.
