...
namespace Chatter.MessageBrokers.AzureServiceBus.Tests.Integration
{
    ...
    // returns this so a test can assert on both the message that round-tripped through Azure Service Bus and
    // the broker context Chatter built for it.
    ...
    //
    // ShouldThrow drives the deadletter scenario: when set, the handler throws after recording, so Chatter's
    ...
    // HandlerSignal<TMessage> from the registry. Registered explicitly by ChatterPipelineHarness as a closed
