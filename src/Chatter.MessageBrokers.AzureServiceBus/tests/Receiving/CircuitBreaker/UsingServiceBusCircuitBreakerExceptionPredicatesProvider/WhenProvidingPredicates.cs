...
namespace Chatter.MessageBrokers.AzureServiceBus.Tests.Receiving.CircuitBreaker.UsingServiceBusCircuitBreakerExceptionPredicatesProvider
{
    ...
    public class WhenProvidingPredicates : Testing.Core.Context
    {
        private readonly Predicate<Exception>[] _predicates =
            new ServiceBusCircuitBreakerExceptionPredicatesProvider().GetExceptionPredicates().ToArray();
