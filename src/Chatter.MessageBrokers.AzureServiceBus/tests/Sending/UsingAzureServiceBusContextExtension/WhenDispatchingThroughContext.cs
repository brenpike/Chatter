...
namespace Chatter.MessageBrokers.AzureServiceBus.Tests.Sending.UsingAzureServiceBusContextExtension
{
    // Characterization tests pinning the CURRENT behavior of the AzureServiceBus() extension on
    // IMessageHandlerContext BEFORE the dispatcher type is collapsed (refactor #126 / STEP-002).
    // Locals are bound via var / IMessageBrokerContext so they survive the upcoming return-type
    // change with at most a trivial edit; nothing here binds to IAzureServiceBusContextDispatcher.
    public class WhenDispatchingThroughContext : Testing.Core.Context
    {
        ...
        [Fact]
        public void MustStampAzureServiceBusInfrastructureTypeOnRealContext()
        {
            IMessageBrokerContext context = CreateRealContext();

            context.AzureServiceBus();
        ...
        {
            ((IMessageHandlerContext)Mock.Of<IMessageHandlerContext>()).AzureServiceBus()
                .Should().BeNull();
        }
        ...
        public async Task MustForwardSendWithDestinationToContext()
        {
            ...
            await mock.Object.AzureServiceBus().Send(command, "destination-path", options);
        ...
        public async Task MustForwardSendWithoutDestinationToContext()
        {
            ...
            await mock.Object.AzureServiceBus().Send(command, options);
        ...
        public async Task MustForwardPublishWithDestinationToContext()
        {
            ...
            await mock.Object.AzureServiceBus().Publish(@event, "destination-path", options);
        ...
        public async Task MustForwardPublishWithoutDestinationToContext()
        {
            ...
            await mock.Object.AzureServiceBus().Publish(@event, options);
        ...
        public async Task MustForwardPublishBatchToContext()
        {
            ...
            await mock.Object.AzureServiceBus().Publish(events, options);
        ...
        public async Task MustForwardForwardToContext()
        {
            ...
            await mock.Object.AzureServiceBus().Forward("forward-destination");
