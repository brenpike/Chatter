...
Both routes into the retry options are built at the same single construction site, so both are validated: the `RetryPolicy` configuration section and the fluent `WithExponentialDelay(...)`. On the fluent path this **replaces** an anonymous failure rather than adding a check that was missing — a negative backoff already threw, but from `ServiceBusRetryOptions.Delay`'s own setter, and an out-of-range count from `MaxRetries`, naming only an SDK member the operator never wrote. The named exception reports the argument you actually supplied.

The two routes stay deliberately asymmetric about what counts as a value at all, and that asymmetry is intended — please do not harmonize it. In **configuration**, a zero, a negative or a `NaN` still means "use the SDK default for that parameter" and does **not** throw, because only a value greater than zero reads as configured; that is the same fall-through the three rules above describe, and it is what stops retry from being switched off by inference. On the **fluent** path every argument is stated outright, so a negative backoff or an out-of-range retry count is a violation and fails the build. A positive backoff the retry path could not wait — too large to become a `TimeSpan`, rejected by its own SDK setter, or a `MaximumBackoffInSeconds` above the timer's delay ceiling — is a stated value either way, and fails the build from both. `WithNoRetry()` and `RetryPolicy:NoRetry` remain the way to disable retry.
