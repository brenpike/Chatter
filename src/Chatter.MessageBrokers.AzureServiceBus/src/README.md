...
| `RetryPolicy:MaximumRetryCount` | SDK default (`3`) | Maximum retry attempts. Omit the key to keep the SDK default; a stated value is always validated, and a stated `0` is refused with a pointer to `NoRetry`. |
| `RetryPolicy:MinimumBackoffInSeconds` | SDK default (`0.8` s) | Base backoff the exponential delay is calculated from (`ServiceBusRetryOptions.Delay`). Omit the key to keep the SDK default; a stated value is offered to `Delay`'s own setter, which has a non-zero floor, so a stated `0` is refused. |
| `RetryPolicy:MaximumBackoffInSeconds` | SDK default (`60` s) | Ceiling on the delay between attempts (`ServiceBusRetryOptions.MaxDelay`). Omit the key to keep the SDK default; a stated value is validated, and `MaxDelay` accepts any non-negative number of seconds, so a stated `0` is accepted. |
...
| Any other populated `RetryPolicy` | `Mode = Exponential`, with `MaxRetries`, `Delay` and `MaxDelay` taken from `MaximumRetryCount`, `MinimumBackoffInSeconds` and `MaximumBackoffInSeconds` respectively — each one whenever its key was stated, and the SDK default for that parameter when the key was omitted. Every stated value is validated. |

**Retry can be switched off from configuration only through the explicit `NoRetry` opt-in.** An omitted key means "use the SDK default" — it does not mean "off". A *stated* `MaximumRetryCount` of `0` means neither: the build fails with a violation naming the `NoRetry` opt-in, because a zero written into configuration is as likely a leftover key as an intention and retry is never switched off by inference. This is a deliberate change: an earlier version inferred "disable retry" from an all-zero `RetryPolicy` section, so an operator who wrote zeros expecting retry to be off is now told to set `NoRetry` to `true` rather than discovering the difference in production.

An absent key and a stated one are distinguishable because every numeric parameter of the `RetryPolicy` section is nullable. Absent means the key was never written: the SDK default for that parameter stands and nothing is validated. Stated means an operator wrote it: it is always validated, so a value the SDK could not run with is named in the failure instead of being silently replaced by that same default.
...
Retry values the SDK could never run with are rejected while the options are being built, by `ServiceBusRetryOptionsValidationException` (namespace `Chatter.MessageBrokers.AzureServiceBus.Options`). The rules are the SDK's own and the runtime's own: the retry count must fall within `0` through `100`; each backoff must be a finite, non-negative number of seconds no larger than `TimeSpan.MaxValue.TotalSeconds`; each backoff must also be one its own SDK setter accepts — `MinimumBackoffInSeconds` reaches `ServiceBusRetryOptions.Delay`, whose accepted range is today `1` millisecond through `5` minutes, and each value is offered to that setter before the options are built so the SDK stays the authority on its own range rather than having it restated here; and `MaximumBackoffInSeconds` must be no larger than the longest delay a .NET timer can wait out, `uint.MaxValue - 1` milliseconds (`4294967` whole seconds, roughly 49.7 days), because every computed retry wait is clamped to `MaxDelay` and then waited out on such a timer. The exception's message, and its `Violations` list, name **every** offending value in one aggregated failure, so an operator who corrects one value does not then discover the next on the following deployment.

Both routes into the retry options — the `RetryPolicy` configuration section and the fluent `WithExponentialDelay(...)` — are built at the same single construction site, so whichever route supplies the options is validated. Only that one is: resolution picks a single source, so a configured section a fluent call overrode is never built and never checked. On the fluent path this **replaces** an anonymous failure rather than adding a check that was missing — a negative backoff already threw, but from `ServiceBusRetryOptions.Delay`'s own setter, and an out-of-range count from `MaxRetries`, naming only an SDK member the operator never wrote. The named exception reports the argument you actually supplied.

The two routes now agree on what counts as a value: a negative, a `NaN`, an out-of-range retry count, or a backoff the retry path could not wait is a stated value on either route and fails the build from either. The only remaining asymmetry is a narrow one, about zero. A fluent `WithExponentialDelay(0, ...)` yields `MaxRetries = 0` and is honored, because every argument on the fluent path is written out by the caller in the same expression. A configured `MaximumRetryCount` of `0` is refused instead, with a pointer to the `NoRetry` opt-in — the no-disable-by-inference rule was only ever scoped to configuration, where a leftover key looks exactly like an intended one. `WithNoRetry()` and `RetryPolicy:NoRetry` remain the way to disable retry from either route.

**Where each `ServiceBusOptions` key is checked.** The `RetryPolicy` keys are the ones checked while the options are built, as above — they are also the only keys this section newly binds. The non-retry keys are checked where the value is used rather than where it is bound: `MaxConcurrentCalls` is checked when a receiver initializes, which raises an `InvalidOperationException` naming the receiver and the offending value when it is below `1`, and `PrefetchCount` is handed to the Azure SDK's `ServiceBusReceiverOptions.PrefetchCount` when a receiver is created, that setter being the authority on what it accepts. `DeltaBackoffInSeconds` is checked nowhere, because it is ignored everywhere — see above.
