...
Retry values the SDK could never run with are rejected while the options are being built, by `ServiceBusRetryOptionsValidationException` (namespace `Chatter.MessageBrokers.AzureServiceBus.Options`). The rules are the SDK's own: the retry count must fall within `0` through `100`, and each backoff must be a finite, non-negative number of seconds no larger than `TimeSpan.MaxValue.TotalSeconds`. The exception's message, and its `Violations` list, name **every** offending value in one aggregated failure, so an operator who corrects one value does not then discover the next on the following deployment.
