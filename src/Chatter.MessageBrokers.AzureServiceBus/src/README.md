...
| The Service Bus section exists, but has no `RetryPolicy` subsection or an empty one | An explicit `ServiceBusRetryOptions` carrying the SDK defaults: `Mode = Exponential`, `MaxRetries = 3`, `Delay = 0.8 s`, `MaxDelay = 60 s`. |
