...
- The Azure Service Bus `IMessagingInfrastructure` registration factory no longer hands the caller an already-disposed receiver. Previously each factory delegate opened a DI scope, resolved the receiver or sender from it, and disposed that scope as it returned — so the resolved instance was disposed before the caller ever used it, and the graph the scope bounded escaped the scope that owned it. The delegates now open no scope and resolve from the root provider: `ServiceBusMessageSender` moves from scoped to transient registration to match, and `ServiceBusReceiver` is no longer a container-registered service at all — every dependency of both types is a singleton, so a transient instance resolved from the root provider has no scoped member to strand (#376).
- The Azure Service Bus receiver now has a single disposal owner. Previously the container also disposed a container-resolved `ServiceBusReceiver` independently of the component whose lifetime bounds the receive pump, so a host disposal that never stopped the pump — the path a startup-fatal failure takes — could dispose the receiver while its pump was still running; the pump would then rebuild a fresh AMQP link behind the already-latched disposed flag, and nothing would ever close it. `ServiceBusReceiver` is now constructed via `ActivatorUtilities.CreateInstance`, which does not enlist it for container disposal, so the container is never a disposer of it — the component that bounds the pump is the receiver's only disposer, because only it knows the pump has stopped (#376).
- A teardown close that does not succeed is now reported, however it ends. Previously the synchronous `Dispose` path fired `CloseAsync()` without awaiting or observing it at all, so a failed close became an unobserved faulted task and nothing recorded that the receiver's sessions, renewal loops and armed receives had been left orphaned; and the receive path, discarding a receiver it would rebuild, observed only a FAULTED task — so a cancelled close, which is a close abandoned partway and leaves exactly those same orphans, read as a clean teardown, and a `CloseAsync()` that returned no task at all threw `NullReferenceException` out of the receive path. Both teardown paths now share one close that reports every ending except success — faulted, cancelled, thrown before a task was returned, or no task returned — so an ending nobody has named yet is reported rather than silently treated as a clean close.

The single consumer-visible delta is that new Warning log line when a teardown close does not succeed. Everything else here is internal to the module's registration and teardown paths — no public surface moved.
