...
- `RetryPolicy:NoRetry` — an explicit opt-in that replaces the previous all-zero-`RetryPolicy`-section-infers-disable-retry behaviour. Every numeric knob of the section is now nullable, so an OMITTED key and a STATED one are no longer the same thing: an omitted key falls back to the SDK's own retry default for that parameter and is never validated, while a stated value is always validated. A stated `MaximumRetryCount` of `0` is consequently refused with a violation naming this opt-in, rather than being honoured as "off" or silently replaced by the SDK default — retry is still never disabled by inference, but the operator is now told so instead of finding out in production (#296, #313).
- `ServiceBusRetryOptionsValidationException` (namespace `Chatter.MessageBrokers.AzureServiceBus.Options`) — thrown while `ServiceBusOptions` are being built when the exponential retry values supplied are ones the Azure Service Bus SDK could never run with: a retry count outside the SDK's own `0` through `100`, or a backoff the retry path could not actually wait: one that is not a finite, non-negative number of seconds within `TimeSpan`'s range, one the SDK's own `Delay`/`MaxDelay` setters reject (each backoff is offered to its setter before the options are built, so the SDK is the authority on its own accepted range rather than a range restated here), or a `MaximumBackoffInSeconds` above the longest delay a .NET timer can wait out (`uint.MaxValue - 1` milliseconds, roughly 49.7 days) — every computed retry wait is clamped to `MaxDelay` and then waited out on such a timer, so a larger value passed the build and then threw `ArgumentOutOfRangeException` from the retry path in place of the transient failure it was retrying. Its message and its `Violations` list name every offending value in one aggregated failure, so an operator who corrects one value does not then discover the next on the following deployment. Both routes into the retry options are covered, because both are now built at one guarded construction site: the `RetryPolicy` configuration section and the fluent `WithExponentialDelay(...)`. On the fluent path this REPLACES an anonymous failure rather than adding a check that was missing — a negative backoff already threw, but from `ServiceBusRetryOptions.Delay`'s own setter, and an out-of-range count from `MaxRetries`, naming only an SDK member the operator never supplied (#296).
...
- **The `RetryPolicy` configuration section now binds and takes effect.** `ServiceBusOptions` binds its section into the fluent-defaulted instance rather than replacing it, which is what it shares with how `Chatter.MessageBrokers` 0.19.0 now binds its own — so a populated `RetryPolicy` block, which previously did nothing, now configures the shared client's retry behaviour. **Upgrader risk:** a consumer who set `RetryPolicy` values expecting retry to already be disabled, relying on the old all-zero-section inference, must now set `RetryPolicy:NoRetry` explicitly — otherwise those zero values are refused and the host does not start. See the breaking-change note below (#296, #313).
...
- **The `ServiceBusOptions` configuration bind surface is narrowed to the type's public properties.** The section was previously bound with non-public binding enabled, which also handed the binder the internal-set `RetryOptions` and `TokenCredential` properties; `RetryPolicy` — the one non-public configuration property that has to bind — is now bound explicitly from its own `RetryPolicy` subsection instead. A stray `RetryOptions` key can therefore no longer run the SDK's validating setters during the bind, and a nested `TokenCredential` object can no longer drive the binder into trying to activate an abstract type; both failed with a raw binder exception at host start. No documented key changes meaning (#296).
- **The retry options are now RESOLVED before they are validated.** `ServiceBusOptionsBuilder.Build()` binds the section, carries the configured `RetryPolicy` forward as data, and only at the end — once every fluent override is in hand — resolves the effective `ServiceBusRetryOptions` from the first source that stated any: the fluent call, then the bound section, then the SDK default. A configured section that a fluent `WithNoRetry()` or `WithExponentialDelay(...)` overrides is therefore discarded WITHOUT being validated. Previously validation ran during the bind, so a `RetryPolicy` section the host was never going to use — say a `MaximumRetryCount` of `101` under an explicit `WithNoRetry()` — blocked the host from starting, contradicting this package's documented fluent-wins precedence (#296).
- The two routes into the retry options now agree on what counts as a stated value: a negative, a `NaN`, an out-of-range retry count, or a backoff the retry path could not wait fails the build from either route. One narrow asymmetry remains and is deliberate: a fluent `WithExponentialDelay(0, ...)` yields `MaxRetries = 0` and is honoured, because every argument on that path is written out by the caller, whereas a configured `MaximumRetryCount` of `0` is refused with a pointer to `RetryPolicy:NoRetry`. The no-disable-by-inference rule was only ever scoped to configuration, where a leftover key looks exactly like an intended one (#296).
...
- **A fluent retry call no longer has to survive the configured retry policy it overrides.** `WithNoRetry()` and `WithExponentialDelay(...)` win over a configured `RetryPolicy` section, but validation used to run while the section was being bound, before the fluent override was in hand — so an invalid configured section stopped the host from starting even though nothing would ever have read it. Validation now runs on the resolved options only (#296).

BREAKING — an invalid `RetryPolicy` section now stops the host from starting: **if your `Chatter:Infrastructure:AzureServiceBus:RetryPolicy` section states a value the Azure Service Bus SDK cannot run with — zeros written to switch retry off, or a typo'd negative — the host will now refuse to start**, throwing `ServiceBusRetryOptionsValidationException` from `ServiceBusOptionsBuilder.Build()` with every offending knob named. It previously started, silently, on SDK-default retry.

The remedy depends on what you meant:

- **You wanted retry off.** Set `RetryPolicy:NoRetry` to `true`. That is the only way to disable retry from configuration, and the violation message says so.
- **You wanted the SDK default.** Remove the key. An omitted key falls back to the SDK default for that parameter and is never validated; only a stated one is.
- **You meant a real value.** Correct it. The failure names every offending knob at once, so one deployment clears them all.
