...
- The configuration fall-through for retry is unchanged and remains deliberate: a zero, negative or `NaN` number in the `RetryPolicy` section still reads as "not configured" and leaves the SDK default for that parameter in place rather than failing the build, because only a value greater than zero counts as configured. The fluent `WithExponentialDelay(...)` is deliberately asymmetric — every argument there is stated outright, so a negative is a violation. Only a positive backoff too large to become a `TimeSpan` fails the build from both routes (#296).
