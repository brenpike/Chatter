...
**Service Bus Retry**: ASB-specific Retry recovery policy applied during receiving. Distinct from the retry the Azure SDK client performs on the wire, which is configured either fluently (`WithNoRetry` / `WithExponentialDelay`) or through the `RetryPolicy` section of Service Bus Options (`NoRetry`, `MaximumRetryCount`, `MinimumBackoffInSeconds`, `MaximumBackoffInSeconds`) and is carried onto the single shared client. Every numeric parameter is nullable, so an omitted key and a stated one are distinguishable: an omitted key leaves the Azure SDK default for that parameter, while a stated value is carried to the SDK's own setter, which raises its own failure for a value it cannot run with. An omitted key therefore means "SDK default" and never "off", whereas a stated zero retry count binds faithfully and does mean off. `DeltaBackoffInSeconds` is accepted for configuration compatibility and ignored — the SDK has no per-attempt delta-backoff knob. Fluent call wins over configuration, and when it does the configured section is never turned into retry options at all, so none of its values ever reach the SDK's setters.
...
**No Retry Opt-In**: Operator knob (`RetryPolicy:NoRetry` / `WithNoRetry`) that switches the Azure SDK client's retry off outright by setting its maximum retry count to zero. It is the intention-revealing way to say so, rather than the only way: a `RetryPolicy` section whose numeric parameters are all omitted reads as "not configured" and falls back to the Azure SDK defaults, so retry is never disabled by inference, but a STATED zero retry count is not an inference — it binds faithfully and also yields no retry, on the configured route exactly as the fluent `WithExponentialDelay(0, ...)` does on its own. Issue #423 owns the question of whether a stated zero should keep binding this way. Default: off, meaning retry stays enabled. Fluent call wins over configuration.
