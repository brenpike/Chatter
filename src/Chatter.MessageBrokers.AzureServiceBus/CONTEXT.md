...
**Service Bus Retry**: ASB-specific Retry recovery policy applied during receiving. Distinct from the retry the Azure SDK client performs on the wire, which is configured either fluently (`WithNoRetry` / `WithExponentialDelay`) or through the `RetryPolicy` section of Service Bus Options (`NoRetry`, `MaximumRetryCount`, `MinimumBackoffInSeconds`, `MaximumBackoffInSeconds`) and is carried onto the single shared client. Every numeric parameter is nullable, so an omitted key and a stated one are distinguishable: an omitted key leaves the Azure SDK default for that parameter, while a stated value is carried to the SDK unchanged, which refuses a value it cannot run with. An omitted key therefore means "SDK default" and never "off", whereas a stated zero retry count binds faithfully and does mean off. `DeltaBackoffInSeconds` is accepted for configuration compatibility and ignored — the SDK has no per-attempt delta-backoff knob. Fluent call wins over configuration, and when it does the configured section is never turned into retry options at all, so none of its values is ever handed to the SDK.

**Service Bus Circuit Breaker**: ASB-specific Circuit Breaker recovery policy applied during receiving.

**PeekLock Settlement**: The Azure Service Bus realization of Settlement (Message Brokers context). Only a PeekLock receive owes one: in `ReceiveAndDelete` mode Azure Service Bus removes the delivery on receipt, so acknowledge, negative acknowledge and deadletter all report the **Not Required** Settlement Outcome. Under PeekLock, completing, abandoning or dead-lettering the received message reports **Settled**. When a PeekLock settlement cannot settle the delivery it targets — no received message in the message broker context, or a delivery the receiver can no longer reach because the session holding its lock was released before settlement ran — there is a lock to release and no way to release it; that is reported as the **Failed** Settlement Outcome rather than THROWN, because the absence is deterministic — retrying the same context would find the same absence — so Recovery must not retry it. A settle call that returns is NOT evidence of a settlement: the receiving infrastructure reports which of settled, not-owed and unreachable actually happened, and only a settlement that reached Azure Service Bus reports **Settled**.
...
**No Retry Opt-In**: Operator knob (`RetryPolicy:NoRetry` / `WithNoRetry`) that switches the Azure SDK client's retry off outright by setting its maximum retry count to zero. It is the intention-revealing way to say so, rather than the only way: a `RetryPolicy` section whose numeric parameters are all omitted reads as "not configured" and falls back to the Azure SDK defaults, so retry is never disabled by inference, but a STATED zero retry count is not an inference — it binds faithfully and also yields no retry, on the configured route exactly as the fluent `WithExponentialDelay(0, ...)` does on its own. That binding is deliberate and settled: zero retries is legitimate operator intent, so a stated zero keeps binding faithfully rather than being rewritten into this opt-in on the operator's behalf, and this knob remains the intention-revealing way to say the same thing. Default: off, meaning retry stays enabled. Fluent call wins over configuration.

**Built Options**: the Built Options invariant defined in the Message Brokers context, applied here to Service Bus Options. `ServiceBusOptionsBuilder.Build()` seeds the defaults, binds the configuration section over them — carrying the configured `RetryPolicy` forward as data only — applies the fluent sentinels, then resolves the retry options from the first source that stated any, and only then registers the finished instance for BOTH the concrete `ServiceBusOptions` type AND the `IOptions<ServiceBusOptions>`, `IOptionsSnapshot<ServiceBusOptions>` and `IOptionsMonitor<ServiceBusOptions>` facets, so every SINGLE-INSTANCE resolution — the concrete type and all three facets — returns that same instance and the container's own options factory never builds a second one. A second `Build()` on the same service collection leaves the earlier instance reachable through `IEnumerable<ServiceBusOptions>` alone, and that instance is itself the finished product of an earlier `Build()`, never a half-built one. Registration runs last, after the connection-string guard and the fluent overrides, so no facet can observe a half-built instance. This builder never registered a `Configure<ServiceBusOptions>`, so nothing divergent existed to close: registering the facets is an ADDED guarantee here, not the repair of a defect this builder introduced. A consumer's own `Configure<ServiceBusOptions>` is consequently no longer consulted.
