...
> **A queue name containing a dot needs pre-bracketing.** `WithChangeFeedQueueName` reaches the receiver's
> `RECEIVE` statement, where it is bracket-quoted rather than interpolated raw: a dotted value (e.g.
> `dbo.MyQueue`) is read as `schema.queue` and each part is quoted separately. A one-part queue name that
> legitimately contains a dot must be bracketed in configuration — `WithChangeFeedQueueName("[my.queue]")`
> — which is then passed through unsplit instead of being split on the dot. An already-bracketed name is
> accepted verbatim, so existing pre-bracketed configuration keeps working unchanged.
