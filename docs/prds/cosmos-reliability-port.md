...
Evolve the reliability port to Two-Tier Reliability: a Relational Tier (the existing ambient-transaction, polling-dispatch model, behavior-unchanged) and a new Document Tier (NoSQL stage-then-commit). Both tiers share one seam — the enqueue contract, the inbox contract, and the transaction-context Atomic-Write Handle. A new Cosmos reliability provider lets an application atomically persist its aggregate and its outbound messages in a single partition-scoped batch; the framework owns the batch lifecycle (open + execute) while the application owns aggregate serialization and partition selection, and the handler contributes aggregate ops to the framework-owned batch (framework-owns-batch-lifecycle). A change-feed Outbox Relay drains persisted outbox records — identified by an outbox discriminator — and publishes them through the broker, never deriving events from domain-document changes. Inbox idempotency uses a co-resident inbox marker contributed by the framework to the same batch before the handler runs; a create-conflict detected at the single framework-owned batch-execute fails the whole batch atomically, making dedup atomic with the aggregate write. The existing EntityFramework users are unaffected at runtime.
...
- The Document Tier uses framework-owns-batch-lifecycle: the framework (outermost document-tier pipeline behavior, sibling of `UnitOfWorkBehavior`) owns batch open and batch-execute; the application owns aggregate serialization and partition selection; the handler contributes aggregate ops to the framework-owned batch; the framework contributes the inbox marker and the outbox record. This keeps the messaging library ignorant of application domain types and container ownership, and makes aggregate-plus-outbox co-location structurally enforced. The handler must NOT execute the batch — batch-lifecycle ownership belongs to the framework; executing the batch inside the handler re-admits the commit-point failure that framework-owns-batch-lifecycle eliminates.
...
- Idempotency and concurrency model on the Document Tier: optimistic concurrency for aggregate writes; conflict-on-create dedup for outbox identity; inbox dedup via a co-resident inbox marker contributed by the framework to the framework-owned batch before the handler runs — a create-conflict detected at the single framework-owned batch-execute fails the whole batch atomically (no aggregate write, no outbox record), making dedup atomic with the handler's write rather than a sequential guard. Applicable only when the message deterministically maps to one aggregate partition. Post-delivery TTL purge requires the application's container to have TTL enabled (container `defaultTtl` set).
...
The following updates must be applied to the epic-#215 child issues to align them with the framework-owns-batch-lifecycle re-decision. GitHub issue edits are out of this docs PR's scope; this PRD is the authoritative record.

- **#220 (Cosmos inbox dedup)**: Revise so inbox dedup is atomic with the aggregate and outbox via the single framework-owned batch-execute — not a standalone inbox write. The inbox marker is contributed by the framework before `next()`; the 409 is detected at batch-execute time after `next()` returns; the all-or-nothing result means no aggregate or outbox write commits on conflict.
- **#218 (skeleton + DI)**: Note the new outermost Document-Tier Batch-Lifecycle Behavior in the DI/registration surface (sibling of `UnitOfWorkBehavior`); its registration is part of the Cosmos provider's DI setup.
- **#219 (outbox enqueue)**: Replace any "app batch" or "handler-owned batch" wording with "the framework-owned batch the handler contributes aggregate ops to"; the handler resolves the batch from `TransactionContext.Container` and adds aggregate ops, but does not open or execute the batch.
- **#222 (change-feed relay)**: Add acceptance criteria for (a) at-least-once relay semantics — a publish can succeed then the delivered-status/TTL update fail, causing redelivery on the next change-feed pass — with downstream consumer idempotency as a requirement; and (b) container-TTL-enabled prerequisite — post-delivery TTL purge requires the application container's `defaultTtl` to be set.
- **#223 (integration suite)**: Inbox-atomicity test must assert the framework-owned batch-execute as the commit point — not a handler-internal execute. The test drives Chatter's public contracts and observes the absence of committed writes on a 409 conflict detected at the framework-owned batch-execute.
