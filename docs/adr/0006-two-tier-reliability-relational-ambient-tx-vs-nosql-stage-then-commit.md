...
- **Withdrawn.** This reason read: `OutboxProcessingBehavior` awaits `next()` and then calls `ProcessBatch`, which dispatches each row inside its own `IUnitOfWork.ExecuteAsync` on `BrokeredMessageOutbox`; nested inside the outer transaction, `BeginAsync` returns the **existing** transaction rather than opening a new one, so the inner `CompleteAsync` would commit the **outer** transaction mid-pipeline. The nesting description still holds and `BeginAsync` still returns the existing transaction, but the consequence no longer does: `UnitOfWork<TContext>` commits only a transaction the same invocation began, so a nested unit of work adopts the outer transaction and leaves it open. See the commit-ownership amendment below. **The canonical order `[OutboxProcessingBehavior, UnitOfWorkBehavior, InboxBehavior]` is unchanged and remains required** — the ordering now rests on the second reason alone, which this ADR already states is independently sufficient.
