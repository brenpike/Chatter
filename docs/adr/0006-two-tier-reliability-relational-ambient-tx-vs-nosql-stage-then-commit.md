...
**Why outbox processing must sit outside the unit of work.** Two independent reasons, either of which is sufficient:

- `OutboxProcessingBehavior` awaits `next()` and then calls `ProcessBatch`, which dispatches each row inside its own `IUnitOfWork.ExecuteAsync` on `BrokeredMessageOutbox`. Nested inside the outer transaction, `BeginAsync` returns the **existing** transaction rather than opening a new one, so the inner `CompleteAsync` would commit the **outer** transaction mid-pipeline.
...
**The deferral.** `UnitOfWork<TContext>.CompleteAsync` calls `SaveChangesAsync` with the default `acceptAllChangesOnSuccess: true` **before** `CommitAsync`. A commit failure inside a retrying execution strategy therefore leaves entries that EF has already accepted as unchanged while the database rolled them back, and the retry never reissues them. That is tracked as **issue #484** and is deliberately not addressed here. Its impact is bounded: it requires an opt-in retrying execution strategy (`EnableRetryOnFailure`), EF Core's default SQL Server strategy does not retry — so default-strategy consumers have no window at all — and the loss is limited to mutation-style handler writes. This change **narrows** that window, from two saves per attempt to one, because the outbox no longer contributes a save of its own inside the strategy. It does **not** close it.
